...
final public class AddPositionListenerImpl implements AddPositionListener {
    ...
    public AddPositionListenerImpl(Orm orm, EventBus eventBus) {
    ...
    public void handle(Message<JsonObject> event) {
        JsonObject position = event.body();

        orm
            .findOne(Entities.USER, "r",
                JsonOps.eq("r." + UserModel.userId, event.headers().get(UserModel.userId)),
                ImmutableList.of(
