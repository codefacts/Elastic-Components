...
import elasta.orm.json.sql.SqlUtils;
import elasta.orm.json.sql.TestCases;
import elasta.orm.json.sql.core.ColumnSpec;
import elasta.orm.json.sql.core.JoinSpec;
import elasta.orm.json.sql.core.TableSpec;
...
public class SqlQueryGeneratorImpl implements SqlQueryGenerator {
    ...
    public String sql(String rootTable, List<FieldInfo> fields) {
        List<TblPathFields> fieldsList = translate(rootTable, fields);
        String select = sqlSelect(rootTable, fieldsList);
        String from = sqlFromAndJoin(rootTable, fieldsList);
        return "select " + select + " from " + from;
    }

    public List<TblPathFields> translate(String table, List<FieldInfo> fields) {
    ...
    public String sqlSelect(String rootTable, List<TblPathFields> fields) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if (ff.getFields() == null || ff.getFields().isEmpty()) {

                addAllFieldsRecursive(tableSpecMap.get(ff.getTable()).getColumnSpecs(), ff.getAlias(), sqlBuilder);

            } else if (ff.getFields().get(0).equals("*")) {

                List<ColumnSpec> columnSpecs = tableSpecMap.get(ff.getTable()).getColumnSpecs();
                addFields(columnSpecs, ff.getAlias(), sqlBuilder);

            } else if (ff.getFields().get(0).equals("**")) {

                List<ColumnSpec> columnSpecs = tableSpecMap.get(ff.getTable()).getColumnSpecs();
                for (ColumnSpec columnSpec : columnSpecs) {
                    sqlBuilder.append(ff.getAlias() + "." + columnSpec.getColumnName() + COMMA);
                ...
                for (int j = 0; j < ff.getFields().size(); j++) {
                    String field = ff.getFields().get(j);
                    sqlBuilder.append(ff.getFields()).append(".").append(field).append(COMMA);
                }
            }
        }
    ...
    public String sqlFromAndJoin(String rootTable, List<TblPathFields> fields) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if (ff.getPath() != null && not(ff.getPath().isEmpty())) {
                count += joinPath(ff.getPath(), rootTable, sqlBuilder);
            }

            if (ff.getFields() == null || ff.getFields().isEmpty()) {

                count += joinAllFieldsRecursive(tableSpecMap.get(ff.getTable()).getColumnSpecs(), ff.getAlias(), sqlBuilder);
                ...
                for (ColumnSpec columnSpec : columnSpecs) {
                    JoinSpec joinSpec = columnSpec.getJoinSpec();
                    if (joinSpec != null) {
                        join(joinSpec, ff.getAlias(), columnSpec.getColumnName(), sqlBuilder);
                        count++;
                    }
                }
            }
    ...
    public static void main(String[] args) {
        ...
        String users = generator.sql("contact", ImmutableList.of(
