 ...
 */
public class IntelligentConverterTransformation implements Transformation<JsonObject, JsonObject> {
    ...
    public JsonObject transform(JsonObject json) {
        ...
        json.getMap().keySet().forEach(key -> {
            if (converters.get(key) != null) {
                ...
                String[] split = Util.normalizeCamelOrSnake(key).toLowerCase().split("\\s");
                if (longType(split)) {
